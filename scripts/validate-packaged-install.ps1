...
function Invoke-LaunchCheck {
  ...
  try {
    ...
    $result.newProcessIds = $newProcessIds
    if ($after.Count -eq 0) {
      throw "启动后未找到进程 $ProcessName"
    }
...
try {
  ...
  try {
    $peOutput = @(node scripts/verify-pe.js $Executable $Architecture 2>&1)
  ...
  if ($failures.Count -eq 0) {
    ...
    try {
      ...
      Write-Evidence 'installer-exit.txt' "exitCode=$installerExitCode"
      ...
      foreach ($line in $logLines) {
        ...
        if ($line -match 'SHA-256 校验通过') {
          ...
          if ($currentProduct -eq 'codex') { $codexChecksumCount += 1 }
        }
      }
      ...
      if ($claudeChecksumCount -ne 1 -or $codexChecksumCount -ne 1) {
        throw "Expected one Claude and one Codex SHA-256 success record; got Claude=$claudeChecksumCount Codex=$codexChecksumCount"
      }
  ...
  try {
    ...
    Write-Evidence 'package-query.txt' @(
      'Claude:'
      ($claudePackage | Format-List Name, PackageFullName, PackageFamilyName, InstallLocation, Status | Out-String).TrimEnd()
      'OpenAI.Codex:'
      ($codexPackage | Format-List Name, PackageFullName, PackageFamilyName, InstallLocation, Status | Out-String).TrimEnd()
    )
    ...
    if ($claudePackage.Count -eq 1) {
      ...
      Assert-Value (-not [string]::IsNullOrWhiteSpace($claudePackage[0].InstallLocation) -and (Test-Path -LiteralPath $claudePackage[0].InstallLocation)) 'Claude install location' "actual=$($claudePackage[0].InstallLocation)"
    }
    if ($codexPackage.Count -eq 1) {
      Assert-Value ($codexPackage[0].Status -eq 'Ok') 'Codex package status' "actual=$($codexPackage[0].Status)"
      Assert-Value (-not [string]::IsNullOrWhiteSpace($codexPackage[0].InstallLocation) -and (Test-Path -LiteralPath $codexPackage[0].InstallLocation)) 'Codex install location' "actual=$($codexPackage[0].InstallLocation)"
    }

    $claudeApplication = $null
    $codexApplication = $null
    if ($claudePackage.Count -eq 1) {
      $claudeManifest = Get-AppxPackageManifest -Package $claudePackage[0]
  ...
  try {
    ...
    Assert-Value ($ccSwitchStartApp.Count -eq 1) 'CC Switch AppID' "expected=com.ccswitch.desktop actual=$($ccSwitchStartApp.AppID -join ',')"
  } catch {
